...
def text_to_speech(text, output_file_path, speaker_wav: str | None = None):
    kwargs = {"speaker_wav": speaker_wav} if speaker_wav else {}
    _get_tts_engine().tts_to_file(text=text, file_path=str(output_file_path), **kwargs)
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, segment_speaker_wav: str | None) -> tuple[int, bytes | None]:
            ...
            return idx, _synthesize_raw(engine, text, wav_path, speaker_wav=segment_speaker_wav)
